		...
		public void bindToMediaPlayer(IMediaPlayer mp) {
			...
			if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN)
				...
				if (surfaceTexture != null) {
					...
					textureHolder.setSurfaceTextureHost(
							mTextureView.mSurfaceCallback);
				}
			} else {
	...
	private static final class SurfaceCallback
			...
			if (mSurfaceTexture != null) {
				if (surfaceHolder == null)
					surfaceHolder = new InternalSurfaceHolder(
							mWeakRenderView.get(), mSurfaceTexture, this);
		...
		public void onSurfaceTextureSizeChanged(SurfaceTexture surface,
			...
			ISurfaceHolder surfaceHolder = new InternalSurfaceHolder(
					mWeakRenderView.get(), surface, this);
			for (IRenderCallback renderCallback : mRenderCallbackMap.keySet()) {
				renderCallback.onSurfaceChanged(surfaceHolder, 0, width,
						height);
			}
		}
		...
		public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
			...
			Log.d(TAG, "onSurfaceTextureDestroyed: destroy: "
					+ mOwnSurfaceTexture);
		...
		public void releaseSurfaceTexture(SurfaceTexture surfaceTexture) {
			if (surfaceTexture == null) {
				...
				if (surfaceTexture != mSurfaceTexture) {
					Log.d(TAG,
							"releaseSurfaceTexture: alive: release different SurfaceTexture");
					surfaceTexture.release();
				} else if (!mOwnSurfaceTexture) {
					Log.d(TAG,
					...
					Log.d(TAG,
							"releaseSurfaceTexture: alive: will released by TextureView");
				}
			}
		}
